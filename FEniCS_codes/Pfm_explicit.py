...
''' Sample code to solve dynamic phase field fracture problem with Explicit central difference scheme'''
...
G_c = Constant(0.025)                                                                      # Fracture toughness
l_0 = Constant(0.1)                                                                        # Length scale parameter
...
# Form equation for equation of motion [M]{a_n} + [K][u_n] = F_n
...
phi_problem = LinearVariationalProblem(lhs(form_phi),rhs(form_phi),pold,bcs=None)  # Phase field equation
...
folder_name = 'Pfm_explicit_results'
os.mkdir(folder_name)

file = open(os.path.join(folder_name,'Results.csv'),'w')    # File to store Energy and displacement
...
plt.savefig(os.path.join(folder_name,'Kinetic Energy'))
...
plt.savefig(os.path.join(folder_name,'Elastic Energy'))
...
plt.title("Work")
plt.savefig(os.path.join(folder_name,'Work'))
...
plt.savefig(os.path.join(folder_name,'Energies'))

plt.figure()
p1 = plot(pold,title='Phase field')
plt.colorbar(p1)
plt.savefig(os.path.join(folder_name,'Phase field'))
...
plt.colorbar(p3)
plt.savefig(os.path.join(folder_name,'U_y'))
...
file1 = open(os.path.join(folder_name,'Phi_uy_Results.csv'),'w')                  # File to store Energy and displacement
...
plt.savefig(os.path.join(folder_name,'y vs u_y'))
...
plt.savefig(os.path.join(folder_name,'y vs phi'))
