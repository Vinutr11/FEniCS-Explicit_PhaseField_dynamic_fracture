...
'''Code to solve dynamic structural problem with central difference scheme (Explicit method)'''
...
k = assemble(a)                          # Assembling LHS k = [M]/dt^2
...
for (i,t) in enumerate(time):         
    ...
    f = assemble(L)                   # Assemble the RHS, f = F_n - [K]{U_n} - (1/dt^2)[M]{U_(n-1)-2U_n}  
    ...
    u_tip[i] = u_old(1.,0)[0]                                           # Tip Displacement (x,y) = (1,0)
...
plt.xlabel("Time (s)")
plt.ylabel("$u_{x}(1,0)$")
plt.savefig(os.path.join(folder_name,'Tip displacement'))
