...
'''Code to solve kalthoff winkler problem using hertz contact for a surface contact''' 
# Importing modules
from fenics import *
import numpy as np 
...
V_dom = VectorElement('CG', mesh.ufl_cell(), 1)
R_imp = FiniteElement('R', mesh.ufl_cell(), 0)
...
k_0 = Constant(1e15)/Constant(assemble(1*ds(1)))
...
# Initialization
v_initial = interpolate(Constant((0,0,32)),W)
u_old_.vector()[:] = -dt*v_initial.vector()
bc_bottom.apply(u_old_.vector()) 

def force_vector(u1,u2,v):
   alpha = u1 - u2
   factor = k_0*((alpha + abs(alpha))/2)**(3/2)*v
   return factor

# Equation of motion
form_domain = Mass(rho_domain,a_old(u_dom,u_old_dom,u_old__dom),v_dom) + K(u_old_dom,pold,v_dom) - force_vector(u_old_imp,u_old_dom[0],v_dom[0])*ds(1)
form_imp = Mass(rho_impact,a_old(u_imp,u_old_imp,u_old__imp),v_imp) + force_vector(u_old_imp,u_old_dom[0],v_imp)*ds(1)

form_u = form_domain + form_imp
...
phi_problem = LinearVariationalProblem(lhs(form_phi),rhs(form_phi),pold)
...
folder_name = 'Impact_problem_results'
...
for (i,t) in enumerate(time):
    ...
    assemble(L,tensor=f)
    bc_bottom.apply(f) 
    solver.solve(M,u_new.vector(),f)                                              # solving Equation of motion
    ...
    Imp_KE = assemble(0.5*Mass(rho_impact,v_old(u_new_imp,u_old__imp),v_old(u_new_imp,u_old__imp)))
    dom_KE = assemble(0.5*Mass(rho_domain,v_old(u_new_dom,u_old__dom),v_old(u_new_dom,u_old__dom)))
