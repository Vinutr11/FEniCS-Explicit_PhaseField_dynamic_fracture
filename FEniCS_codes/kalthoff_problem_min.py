...
def displacement_new(u_old,v_old,a_old,ufl=True):
    '''Function to define displacement, U_(n+1) = (U_n + dt*v_n + (dt**2/2)*a_n)'''
    if ufl == True:
        dt_ = dt
    else:
        dt_ = float(dt)
...
def velocity_new(v_old,a_old,a_new,ufl=True):
    '''Function to define velocity, V_(n+1) = (V_n + (a_n + a_(n+1))/(dt/2))'''
    if ufl == True:
        dt_ = dt
    else:
        dt_ = float(dt)
...
def Update_displacement(u_new,u_old,v_old,a_old):
    '''Function to update the displacement'''
...
solver = LUSolver(k,'mumps')                   # Assembling LHS and applying boundary condition for equation of motion
...
for (i,dt) in enumerate(np.diff(time)):
    ...
    velocity.t = time[i]                             # updating velocity load     
    ...
    # Track the position of crack tip
    ...
    l_b.vector()[:] = p_new.vector()                             # Update the lower bound of phase field to take care of irreversibilty condition
